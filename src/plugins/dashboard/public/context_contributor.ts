...
export class DashboardContextContributor implements ContextContributor {
  ...
  async captureStaticContext(): Promise<Record<string, any>> {
    ...
    console.log(
      '🔍 Dashboard: window.dashboardPlugin?.currentDashboardContainer:',
      (window as any).dashboardPlugin?.currentDashboardContainer
    );
  ...
  private async captureContainerContext(container: any): Promise<Record<string, any>> {
    try {
      ...
      console.log(
        `✅ Dashboard: Context captured with ${embeddableContexts.length} embeddables:`,
        context
      );
      return context;
    } catch (error) {
  ...
  private async captureAllEmbeddableContexts(container: DashboardContainer): Promise<any[]> {
    ...
    console.log(
      '🔍 Dashboard: Container input panels:',
      Object.keys(container.getInput().panels || {})
    );
    console.log(
      '🔍 Dashboard: Container output embeddableLoaded:',
      container.getOutput().embeddableLoaded
    );
    ...
    for (const id of childIds) {
      try {
        ...
        console.log(
          ...
          Object.getPrototypeOf(embeddable).constructor.name
        );
        ...
        const embeddableProps = {
          ...
          getSavedObject:
            typeof embeddableAny.getSavedObject === 'function' ? 'Available' : 'Not available',
          getVisualization:
            typeof embeddableAny.getVisualization === 'function' ? 'Available' : 'Not available',
        ...
        const embeddableContext = {
          ...
          metadata: {
            ...
            visualizationType:
              (output as any).visType ||
              (input as any).visState?.type ||
              ((input as any).savedObjectId && embeddable.type === 'visualization'
                ? 'saved_visualization'
                : null),
            // Try to get index pattern information
            indexPatternId:
              (output as any).indexPattern?.id ||
