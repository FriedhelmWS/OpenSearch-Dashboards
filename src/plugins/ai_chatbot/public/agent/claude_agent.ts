...
export class ClaudeOSDAgent {
    ...
    return [
      {
        ...
        parameters: {
          ...
          properties: {
            field: {
              type: 'string',
              description: 'Field name to filter on (e.g., level, service, message)',
            },
            value: {
              type: 'string',
              description: 'Value to filter for (e.g., ERROR, auth-service)',
            },
            operation: {
              type: 'string',
              enum: ['is', 'is_not', 'exists'],
  ...
  private async addFilter(params: {
    field: string;
    value: string;
    operation?: string;
  }): Promise<string> {
