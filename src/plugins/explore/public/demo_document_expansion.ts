 ...
 */
export function simulateFieldFilter(
  fieldName: string,
  filterValue: any,
...
export function testProgrammaticActions() {
  ...
  setTimeout(() => {
    exploreContributor
      .executeAction('ADD_FIELD_FILTER', {
        fieldName: 'level',
        filterValue: 'INFO',
      })
      .then((result: any) => {
...
export function debugContextProvider() {
  ...
  if (exploreContributor) {
    ...
    console.log(
      '📖 Explore Contributor trigger actions:',
      exploreContributor.contextTriggerActions
    );
  }
